...
export default function Quiz({ session }) {
  ...
  const { FinalTestChance, isAgreed } = useContext(TestContext);
                ...
                {QuestionState?.length > 0 &&
                  QuestionOptions[currentQuestion]?.map((answer, index) => (
                    ...
                    >
                      ...
                      />
                      <span className='ml-6 text-skin-base text-md md:tex-lg font-medium '>{answer.answer}</span>
                    </div>
                  ))}
              </div>
