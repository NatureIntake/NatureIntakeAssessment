...
export default function Quiz() {
  ...
  const [QuestionState, setQuestionState] = useState(Qna2);
  const [Question, setQuestion] = useState(null);

  useEffect(() => {
    // const path = "/api/QuestionBank";
    // const res = fetch(
    //   `http://localhost:3000/api/getQuestion/${formData[0].class}/${QuizId}/10`
    // )
    //   .then((res) => res.json())
    //   .then((data) => {
    //     Cookies.set("Question", data, { expires: 60 ^ 3 });
    //   });
    async function fetchQuestion() {
      ...
      Cookies.set("Questions", data, { expires: 60 ^ 3 });
    }
    console.log(Cookies.get("Questions"));
    fetchQuestion();
  }, []);
